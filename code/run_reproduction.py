...
def convert_examples_to_features(js,tokenizer,args, idx=None):
    #source
    ...
    item_idx = js.get('idx', js.get('index', js.get('id', idx)))
    label = js.get('target', js.get('label', 0))
    return InputFeatures(source_tokens,source_ids,item_idx,label)
    ...
    def __init__(self, tokenizer, args, file_path=None, sample_percent=1., w_embeddings=None):
        ...
        with open(file_path) as f:
            ...
            for i, js in enumerate(data):
                self.examples.append(convert_examples_to_features(js, tokenizer, args, idx=i))
...
def evaluate(args, model, tokenizer, eval_dataset=None, eval_when_training=False):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            example_indexes.append(np.array(indx_))
        nb_eval_steps += 1
    
    logits=np.concatenate(logits,0)
    labels=np.concatenate(labels,0)
    example_indexes = np.concatenate(example_indexes, 0)
...
def test(args, model, tokenizer):
    ...
    if args.save_results:
        ...
        df['target'] = labels
        df['seed'] = args.seed
        df['split'] = args.test_split
        df.to_csv(os.path.join(args.result_dir, 'predictions_{}_{}.csv'.format(args.seed, args.test_split)))
